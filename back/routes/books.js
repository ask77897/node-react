...
router.post('/insert', function(req, res){
    ...
    db.get().query(sql, [isbn], function(err, rows){
        if(err) console.log("books.2 : ", err);
        if(rows.length > 0){
            res.send("1");
        }else{
            sql=`insert into books(title, price, authors, contents, publisher, image, isbn) values(?, ?, ?, ?, ?, ?, ?)`;
            db.get().query(sql, [title, price, authors, contents, publisher, image, isbn], function(err, rows){
                if(err) console.log("books.3 : ", err);
                res.send('0');
            });
        }
    });
